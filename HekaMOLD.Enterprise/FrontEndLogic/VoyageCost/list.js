app.controller('voyageCostListCtrl', function sidebarCtrl($scope, $http) {
    DevExpress.localization.locale('tr');
    $scope.voyageCostList = [];
    $scope.voyageList= [];
    // LIST FUNCTIONS
                    ...
                    buttons: [
                        {
                            name: 'preview', cssClass: '', text: 'Düzenle', onClick: function (e) {
                                ...
                                $scope.voyageCostList = HOST_URL + 'VoyageCost/GetVoyageCostList';
                                $scope.voyageList = HOST_URL + 'Voyage/GetVoyageList';
                                $http.get(HOST_URL + 'VoyageCost/BindModelByVoyage?rid=' + e.row.data.Id, {}, 'json')
                                    .then(function (resp) {
                                        if (typeof resp.data != 'undefined' && resp.data != null) {
                                            if (resp.data.Id > 0) {

                                                window.location.href = HOST_URL + 'VoyageCost?rid=' + resp.data.Id + '&vid=' + e.row.data.Id;
                                            }
                                            else {
                                                window.location.href = HOST_URL + 'VoyageCost?rid=' + resp.data.Id + '&vid=' + e.row.data.Id;
                                            }
