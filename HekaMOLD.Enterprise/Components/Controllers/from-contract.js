app.controller('fromContractCtrl', function ($scope, $http) {
    $scope.modelObject = {
        Quantity: 0,
    ...
    $scope.bindReceiptDetails = function () {
        $('#receiptDetailsFrom').dxDataGrid({
            dataSource: {
                load: function () {
                    if ($scope.receiptDetailList.length == 0)
                        $scope.receiptDetailList = $.getJSON(HOST_URL + 'ContractWorks/GetContractDeliveries?workOrderDetailId='
                            + $scope.workOrderDetailId, function (data) {
                            data.forEach(d => {
